...
def keep_top_X_edges_per_node(df, X=20):
    ...
    #remove nodes with many edges (hubs)
    top_edges = df.groupby('v').filter(lambda x: len(x) <= X)

    top_edges = top_edges.groupby('u').apply(lambda x: x.nlargest(15, 'jaccard')).reset_index(drop=True)
    return top_edges
