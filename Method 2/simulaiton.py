...
# --- Keep your provided filtering functions ---
...
# --- Modified Generation Function to accept a fixed similarity ---
def generate_unpruned_world_at_sim(num_species, proteins_per_species, hgt_sim):
    ...
    for i in range(num_species):
        for j in range(i + 1, num_species):
            ...
            for _ in range(250): 
                ...
                edges.append({"u":u, "v":v, "shared_kmers":int(jaccard*1000), 
                              "jaccard":jaccard, "species_u":sp_u, "species_v":sp_v})
    ...
    for _ in range(30):
        ...
        u, v = random.choice(nodes[sp_u]), random.choice(nodes[sp_v])
        tp_nodes.update([u, v])
        edges.append({"u":u, "v":v, "shared_kmers":int(hgt_sim*1000), 
                      "jaccard":hgt_sim, "species_u":sp_u, "species_v":sp_v})
        
    return pd.DataFrame(edges), tp_nodes

def evaluate_pipeline_performance(hgt_sim):
    """Runs one full cycle and returns the AUC."""
    ...
    if len(set(y_true)) < 2: return 0.0 # No TPs survived pruning
    
    fpr, tpr, _ = roc_curve(y_true, y_scores)
    return auc(fpr, tpr)

if __name__ == "__main__":
    # Test similarities from 0.1 to 0.9
    ...
    # Plotting ROC-AUC vs Jaccard Similarity
